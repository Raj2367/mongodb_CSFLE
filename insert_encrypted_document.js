...
const secureClient = new MongoClient(connectionString, {
  ...
  autoEncryption: {
    ...
    extraOptions: {
      ...
      monogocryptdSpawnPath: "/usr/bin/mongocryptd"
    },
  },
});
// end-client
...
async function main() {
  try {
    ...
    try {
      ...
      console.log(
        await regularClient.db(db).collection(coll).findOne({ name: /manoj/ })
      );
      ...
      console.log(
        await secureClient.db(db).collection(coll).findOne({ name: /manoj/ })
      );
      // end-find
    } finally {
